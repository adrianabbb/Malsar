function [ best_param best_param2 perform_mat] = CrossValidation2Param_Logistic...
    ( X, Y, obj_func_str, obj_func_opts, param_range, param_range2, cv_fold, eval_func_str, higher_better)
...
% performance vector
perform_mat = zeros(length(param_range),length(param_range2));
...
for cv_idx = 1: cv_fold
    ...
    for p_idx = 1: length(param_range)
        for p_idx2 = 1: length(param_range2)
            [W,C] = obj_func(cv_Xtr, cv_Ytr, param_range(p_idx), param_range2(p_idx2), 2, obj_func_opts);
             perform_mat(p_idx,p_idx2) = perform_mat(p_idx,p_idx2) + eval_func(Y, X, W, C);
        end
    end
end
...
if( higher_better)
    [~,ind] = max(perform_mat(:));
    [best_idx1,best_idx2] = ind2sub(size(perform_mat),ind);
else
     [~,ind] = min(perform_mat(:));
    [best_idx1,best_idx2] = ind2sub(size(perform_mat),ind);
end

best_param = param_range(best_idx1);
best_param2 = param_range2(best_idx2);
end
