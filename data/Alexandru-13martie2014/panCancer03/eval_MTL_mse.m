function [mse, mse_task] = eval_MTL_mse (Y, X, W)
...
%
    ...
    for t = 1: task_num
        y_pred = X{t} * W(:, t);
        %ada pt classification: y_pred =  exp(X{t} * W(:, t))./(1+exp(X{t} * W(:, t)));
        mse = mse + sqrt(sum((y_pred - Y{t}).^2)) * length(y_pred);
        
        %ada: mse = mse + sqrt(sum((y_pred - Y{t}).^2)) / length(y_pred);
        %ada: sau poate AUC ca si evaluation measure
        total_sample = total_sample + length(y_pred);
        mse_task(k) = sqrt(sum((y_pred - Y{t}).^2));
        %ada: din packetul grecilor testerrs(t) = (Y{t} - y_pred)' * (Y{t} - y_pred) / length(y_pred);
    end
